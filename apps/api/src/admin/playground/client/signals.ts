...
// This is the dummy local-playground key that the server injects into the
// page. It is not a secret and it is not meant to authorize anything outside
// the local bootstrap flow.
// Keep the fallback obviously fake and UUID-shaped so the auth middleware
// accepts it without implying it is a real token.
